...
func (s *Service[U]) generateLegacyKeys() {
	...
	s.access.RLock()
	currentAlterIds := s.alterIds
	currentUpdateTime := s.alterIdUpdateTime
	s.access.RUnlock()

	for user, alterIds := range currentAlterIds {
		beginSec := currentUpdateTime[user]
...
func (s *Service[U]) NewConnection(ctx context.Context, conn net.Conn, source M.Socksaddr, onClose N.CloseHandlerFunc) error {
	...
	s.access.RLock()
	userKeyMap := s.userKey
	userIdCipherMap := s.userIdCipher
	alterIdMap := s.alterIdMap
	alterIdCount := len(s.alterIds)
	s.access.RUnlock()
	...
	var user U
	var found bool
	for currUser, userIdBlock := range userIdCipherMap {
	...
	if !found {
		...
		if currUser, loaded := alterIdMap[decodedId]; loaded {
			found = true
			legacyProtocol = true
			user = currUser.User
