...
type Inbound struct {
	...
	service   *vmess.Service[inbounduser.Identity]
...
func NewInbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.VMessInboundOptions) (adapter.Inbound, error) {
	...
	service := vmess.NewService[inbounduser.Identity](adapter.NewUpstreamContextHandlerEx(inbound.newConnectionEx, inbound.newPacketConnectionEx), serviceOptions...)
	inbound.service = service
	err = service.UpdateUsers(vmessIdentities(options.Users), common.Map(options.Users, func(it option.VMessUser) string {
		return it.UUID
...
func (h *Inbound) UpdateUsers(users []option.VMessUser) error {
	return h.service.UpdateUsers(vmessIdentities(users), common.Map(users, func(it option.VMessUser) string {
		return it.UUID
...
func vmessIdentities(users []option.VMessUser) []inbounduser.Identity {
	return inbounduser.Identities(users, func(it option.VMessUser) string {
		return it.Name
	})
}
